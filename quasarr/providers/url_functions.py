#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# URL Functions für Quasarr Providers
...
def normalize_url(url, base_url=None):
    ...
    Args:
        ...
        base_url (str): Basis-URL für relative URLs
...
def build_search_url(base_url, query, params=None):
    ...
    Args:
        base_url (str): Basis-URL für die Suche
        query (str): Suchbegriff
        params (dict): Zusätzliche Parameter
...
def is_valid_download_link(url):
    """
    Prüft ob eine URL ein gültiger Download-Link ist
    
    Args:
        url (str): Die zu prüfende URL
        
    Returns:
        bool: True wenn gültiger Download-Link
    """
    if not url:
        return False
...
def extract_file_id(url):
    ...
    """
    # Verschiedene Patterns für verschiedene Hoster
    patterns = [
...
def get_final_url(url, session=None, timeout=10):
    """
    Folgt Redirects und gibt die finale URL zurück
    ...
    Returns:
        str: Finale URL oder ursprüngliche URL bei Fehler
    """
    try:
        if session:
...
def get_url_headers(url=None):
    """
    Gibt Standard-HTTP-Headers zurück
    
    Args:
        url (str): Optional URL für spezifische Headers
    ...
    # Spezifische Headers für bestimmte Domains
    if url:
        domain = extract_domain(url)
