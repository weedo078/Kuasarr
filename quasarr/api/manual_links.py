...
from quasarr.providers.html_templates import render_button, render_centered_html, render_fail
from quasarr.storage.manual_jobs import (
...
def _page_header(subtitle: str) -> str:
    return (
        f'<h1><img src="{images.logo}" type="image/png" alt="Quasarr logo" class="logo"/>Quasarr</h1>'
...
def setup_manual_link_routes(app: Bottle) -> None:
    ...
    def _overview_page(message: Optional[str] = None) -> str:
        ...
        for job in jobs:
            ...
            path = job.get("download_path") or "Default (Quasarr/<jd:packagename>)"
        ...
        actions = (
            ...
            f"{render_button('Submit Manual Links', 'primary', {'onclick': \"location.href='/manual-links/new'\"})}"
            f"{render_button('Refresh', 'secondary', {'onclick': \"location.href='/manual-links'\"})}"
            f"{render_button('Back', 'secondary', {'onclick': \"location.href='/'\"})}"
            "</div>"
        )
    ...
    def _detail_page(job: Dict[str, Any], events: List[Dict[str, Any]], message: Optional[str] = None) -> str:
        ...
        events_list = "".join(
            "<li>"
            f"<strong>{_escape(evt.get('timestamp'))}</strong> — {_escape(evt.get('type'))}"
        ...
        status_html = _status_badge(job.get("status"))
        path_display = job.get("download_path") or "Default (Quasarr/<jd:packagename>)"
        ...
        if job.get("status") == STATUS_AWAITING_CAPTCHA or protected_packages:
            actions.append(
                f"<div class='inline-action'>{render_button('Open CAPTCHA Helper', 'primary', {'onclick': \"location.href='/captcha'\"})}</div>"
            )
