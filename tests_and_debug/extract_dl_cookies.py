...
Extrahiert Cookies aus Browser-Datenbanken und stellt sie für Quasarr bereit
"""
...
class BrowserCookieExtractor:
    ...
    def save_cookies_for_quasarr(self, output_file=None):
...
def main():
    ...
    if extractor.extract_all_cookies():
        if extractor.save_cookies_for_quasarr():
            desktop_path = extractor.get_desktop_path()
            print("\n✅ Erfolgreich! Die Cookie-Datei wurde auf dem Desktop gespeichert.")
            print(f"📍 Speicherort: {os.path.join(desktop_path, 'dl_cookies.json')}")
            print("\n📋 Nächste Schritte:")
            print("1. Die Datei 'dl_cookies.json' liegt nun auf dem Desktop")
            print("2. Kopiere sie in den Quasarr-Ordner (siehe Docker-Compose)")
