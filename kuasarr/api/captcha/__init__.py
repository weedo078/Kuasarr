...
from kuasarr.downloads.linkcrypters.filecrypt import get_filecrypt_links
try:
    import kuasarr.downloads.manual_jobs as manual_jobs
except ModuleNotFoundError as exc:  # pragma: no cover - defensive safety net for mismatched installs
    raise ImportError(
...
def setup_captcha_routes(app):
    ...
    def proxy_html(captcha_id):
        captcha_base = captcha_values()["url"]
        target_url = f"{captcha_base}/captcha/{captcha_id}.html"
    ...
    def proxy_json(captcha_id):
        captcha_base = captcha_values()["url"]
        target_url = f"{captcha_base}/captcha/{captcha_id}.json"
    ...
    def proxy_pngs(captcha_id, uuid, filename):
        captcha_base = captcha_values()["url"]
        new_url = f"{captcha_base}/captcha/{captcha_id}/{uuid}/{filename}"
    ...
    def proxy_check(captcha_id):
        captcha_base = captcha_values()["url"]
        new_url = f"{captcha_base}/captcha/{captcha_id}/check"
