...
from kuasarr.providers.myjd_api import Myjdapi, TokenExpiredException, RequestTimeoutException, MYJDException, Jddevice
...
def extract_valid_hostname(url, shorthand):
    try:
        ...
        if domain.startswith(".") or domain.endswith(".") or "." not in domain[1:-1]:
            ...
            message = (
                f'Error: "{domain}" is a fileâ€‘hosting domain and cannot be used here directly! '
    ...
    settings_to_enforce = [
        {
            ...
            "expected_value": "ALWAYS",  # Downloads must start automatically for kuasarr to work
        },
        {
            "namespace": "org.jdownloader.settings.GeneralSettings",
            "storage": None,
        ...
        },
        {
            ...
            "expected_value": "NEVER",  # Links must be kept after download for kuasarr to work
        },
        {
        ...
        },
        {
            ...
            "expected_value": False,  # Links must be kept after extraction for kuasarr to work
        },
        {
        ...
        },
        {
            ...
            "expected_value": "INCLUDE_OFFLINE",  # Offline links must always be kept for kuasarr to handle packages
        },
        {
            "namespace": "org.jdownloader.gui.views.linkgrabber.addlinksdialog.LinkgrabberSettings",
            "storage": None,
        ...
        },
        {
            ...
            "expected_values": [
                {'conditionFilter':
                 ...
                 'matchAlwaysFilter': {'enabled': False}, 'name': 'kuasarr_Block_Files',
...
def match_in_title(title: str, season: int = None, episode: int = None) -> bool:
    ...
    pattern = re.compile(
        r"(?i)(?:\.|^)[sS](\d+)(?:-(\d+))?"  # season or seasonâ€‘range
        r"(?:[eE](\d+)(?:-(?:[eE]?)(\d+))?)?"  # episode or episodeâ€‘range
        r"(?=[\.-]|$)"
    )
    ...
    for s_start, s_end, e_start, e_end in matches:
        ...
        if episode is None:
            if not e_start:
                ...
                # title did specify an episode â€” skip this match
                continue
    ...
    try:
        ...
        if is_tv_search:
            ...
            if not SEASON_EP_REGEX.search(title):
                ...
                return False
            # if caller specified a season or episode, doubleâ€‘check the match
        ...
        # unknown search source â€” reject by default
...
def normalize_magazine_title(title: str) -> str:
    ...
    - Handle malformed "DD.YYYY.YYYY" cases (e.g., 04.2006.2025 â†’ 2025 06 04).
    - Convert two-part month-year like "3.25" into YYYY MM.
    - Convert "No/Nr/Sonderheft X.YYYY" when Xâ‰¤12 into YYYY MM.
    ...
    title = re.sub(
        r"\b(?:vom\s*)?(\d{1,2})\.?\s+([A-Za-zÃ„Ã–ÃœÃ¤Ã¶Ã¼ÃŸ]+)\s+(\d{4})\b",
        repl_dmony,
        title,
        flags=re.IGNORECASE
    )
    ...
    title = re.sub(r"\b([A-Za-zÃ„Ã–ÃœÃ¤Ã¶Ã¼ÃŸ]+)\s+(\d{4})\b", repl_mony, title, flags=re.IGNORECASE)
...
def _month_num(name: str) -> int:
    name = name.lower()
    mmap = {
        'januar': 1, 'jan': 1, 'februar': 2, 'feb': 2, 'mÃ¤rz': 3, 'maerz': 3, 'mÃ¤r': 3, 'mrz': 3, 'mae': 3,
